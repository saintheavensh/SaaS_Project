import { pgTable, uuid, integer, text, index, check } from 'drizzle-orm/pg-core';
...
export const stockLedger = pgTable('stock_ledger', {
    ...
    buyPrice: text('buy_price').notNull(), // Added in Step 5 for COGS
    ...
    quantityCheck: check('stock_ledger_quantity_check', sql`${table.quantity} > 0`),
    typeCheck: check('stock_ledger_type_check', sql`${table.type} IN ('IN', 'OUT', 'ADJUSTMENT')`),
